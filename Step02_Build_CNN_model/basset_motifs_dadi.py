...
weblogo_opts = '-F pdf --resolution 300'
...
def main():
    ...
    parser.add_option('-r', dest='meme_rbp', default='%s/data/motifs/rbp_consensus_human.meme' % os.environ['BASSETDIR'], help='MEME database used to annotate RBP motifs')
    ...
    # convert to letters
    seqs = dna_io.vecs2dna(seq_vecs)  #shape (131, 4, 1, 400)
    ...
    filter_weights = np.array(model_hdf5_in['weights']) #shape (50, 4, 5)
    filter_outs = np.array(model_hdf5_in['outs'])       #shape (131, 50, 400)
    model_hdf5_in.close()
    ...
    #################################################################
    # run tomtom
    subprocess.call('/apps/lab/miket/meme/4.11.2/bin/tomtom -dist pearson -thresh 0.1 -oc %s/tomtom %s/filters_meme.txt %s' % (options.out_dir, options.out_dir, options.meme_db), shell=True)
    subprocess.call('/apps/lab/miket/meme/4.11.2/bin/tomtom -dist pearson -thresh 0.1 -oc %s/tomtom_rbp %s/filters_meme.txt %s' % (options.out_dir, options.out_dir, options.meme_rbp), shell=True)
    
    # read in annotations
    #filter_names = name_filters(num_filters, '%s/tomtom/tomtom.txt'%options.out_dir, options.meme_db)
    filter_names = name_filters_dadi(num_filters, '%s/tomtom/tomtom.txt'%options.out_dir, '%s/tomtom_rbp/tomtom.txt'%options.out_dir, options.meme_db, options.meme_rbp)
    ...
    if options.plot_heats:
        ...
        plot_target_corr(filter_outs, seq_targets, filter_names, target_names, '%s/filter_target_cors_mean.pdf'%options.out_dir, '%s/filter_target_cors_mean.txt'%options.out_dir, 'mean')
        plot_target_corr(filter_outs, seq_targets, filter_names, target_names, '%s/filter_target_cors_max.pdf'%options.out_dir, '%s/filter_target_cors_max.txt'%options.out_dir, 'max')
...
def name_filters_dadi(num_filters, tomtom_dna, tomtom_rbp, meme_db_file, meme_rbp_file):
    ''' Name the filters using Tomtom matches.
    ...
    # name by protein
    if tomtom_dna is not None and meme_db_file is not None:
        motif_protein = get_motif_proteins(meme_db_file)

        # hash motifs and q-value's by filter
        filter_motifs = {}

        tt_in = open(tomtom_dna)
        tt_in.readline()
        for line in tt_in:
            a = line.split()
            fi = int(a[0][6:])
    ...
    if tomtom_rbp is not None and meme_rbp_file is not None:
        motif_protein = get_motif_proteins(meme_rbp_file)

        # hash motifs and q-value's by filter
        filter_motifs = {}

        tt_in = open(tomtom_rbp)
        tt_in.readline()
        for line in tt_in:
            a = line.split()
            fi = int(a[0][6:])
        ...
        for fi in filter_motifs:
            top_motif = sorted(filter_motifs[fi])[0][1]
            filter_names[fi] += '_RBP-%s' % motif_protein[top_motif]
